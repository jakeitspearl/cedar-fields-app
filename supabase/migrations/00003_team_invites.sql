-- Allow owners to invite teammates (workers or other owners) without re-bootstrapping a company.
--
-- The signup trigger now branches on raw_user_meta_data.invited_to_company:
--   - If present (set by the Admin API call in our /admin/team server action),
--     create just a profile row that joins the invited user to that existing company.
--   - If absent (a brand-new owner doing self-signup), keep the original bootstrap:
--     new company + seed clients/estimates/jobs/invoices/receipts.
...
begin
  ...
  -- Invited path: skip company creation + seed, just join the existing company
  if invited_company is not null and invited_company <> '' then
    insert into profiles (id, company_id, role, full_name)
    values (new.id, invited_company::uuid, invited_role::user_role, user_name);
    return new;
  end if;

  -- Bootstrap path (self-signup): create company + seed demo data
