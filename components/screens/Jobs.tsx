...
export function JobsScreen({
  ...
  oneTime?: OneTimeJob[]
  tab?: 'recurring' | 'onetime'
  onTabChange?: (tab: 'recurring' | 'onetime') => void
  footerSlot?: React.ReactNode
}) {
  const [localTab, setLocalTab] = useState<'recurring' | 'onetime'>('recurring')
  const tab = tabProp ?? localTab
  const setTab = (t: 'recurring' | 'onetime') => {
    if (onTabChange) onTabChange(t)
    else setLocalTab(t)
  }
  const recurring = recurringProp ?? DATA.recurringJobs
  const oneTime = oneTimeProp ?? DATA.oneTimeJobs

  return (
    <>
      <TopBar title="Jobs" sub={`${recurring.length} recurring · ${oneTime.length} one-time`} />
      <div className="screen-body">
        <button className="new-btn" onClick={onNew} disabled={!onNew}>
        ...
        {tab === 'recurring' &&
          (recurring.length === 0 ? (
            <EmptyState text="No recurring jobs yet." />
          ) : (
            recurring.map((j) => {
              const ws = j.workers ?? []
              return (
                <button key={j.id} className="card" onClick={() => onOpenJob({ ...j, kind: 'recurring' })}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        ...
                        <span style={{ fontWeight: 600 }}>{j.frequency || 'no frequency'}</span>
                      </div>
                    </div>
                  ...
                  >
                    {ws.length === 0 ? (
                      <span style={{ color: 'var(--stone-500)', fontStyle: 'italic' }}>No crew assigned</span>
                    ) : (
                      ws.map((w, i) => (
                        <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                          ...
                          <span>{w}</span>
                          {i < ws.length - 1 && <span style={{ margin: '0 4px', color: 'var(--stone-300)' }}>·</span>}
                        </span>
                      ))
                    )}
                  </div>
        ...
        {tab === 'onetime' &&
          (oneTime.length === 0 ? (
            <EmptyState text="No one-time jobs yet." />
          ) : (
            oneTime.map((j) => (
              <button key={j.id} className="card" onClick={() => onOpenJob({ ...j, kind: 'onetime' })}>
                ...
                >
                  <div style={{ display: 'flex', gap: 4, alignItems: 'center', color: 'var(--stone-600)', flexWrap: 'wrap' }}>
                    {j.workers.length === 0 ? (
                      <span style={{ color: 'var(--stone-500)', fontStyle: 'italic' }}>No crew assigned</span>
                    ) : (
                      j.workers.map((w, i) => (
                ...
                {j.date && <div style={{ fontSize: 11.5, color: 'var(--stone-500)', marginTop: 6 }}>{j.date}</div>}
              </button>
            ))
          ))}
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="card" style={{ padding: 24, textAlign: 'center', color: 'var(--stone-500)', fontSize: 14 }}>
      {text}
    </div>
  )
}
          ...
          <>
            ...
            {job.kind === 'onetime' && (
              <>
                ...
                <div className="card" style={{ cursor: 'default', padding: 14, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {job.workers.map((w, i) => (
                    <div
                      key={i}
                      style={{
